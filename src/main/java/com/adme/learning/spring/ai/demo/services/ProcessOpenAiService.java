...
public final class ProcessOpenAiService extends AbstractEventService {
  ...
  public ProcessOpenAiService(OpenAiChatClient openAiChatClient,
      ...
      RouteMessageService routeMessageService, ProcessMessageService processMessageService) {
  ...
  public void process(Event event) {
    ...
    ResponseDto responseDto = new ResponseDto(null,event.content(),event.producer(),"Anthropic");

    ResponseDto saved = processMessageService.saveMessage(
        ResponseMapper.mapToResponseEntity(responseDto));

    log.info("Anthropic saved message : {}", saved);
